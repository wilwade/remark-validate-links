...
export async function findReferences(ctx) {
  ...
  const config = {
    ...
    // @ts-ignore
    basePath: ctx.options.basePath // FORK ADDED
  }
  /** @type {string[]} */
  const statted = []
  /** @type {string[]} */
...
/**
 * FORKED NOTICE
 * find the absolute path for a relative file
 * @param {string} value
 * @param {string|undefined} root
 * @param {string|undefined} basePath
 */
function findFile(value, root, basePath) {
  let hash;
  [value, hash] = value.split("#");
  hash = hash ? "#" + hash : "";
  const absoluteValue = path.join(root || "", basePath || "", value)
  if (existsSync(absoluteValue)) return absoluteValue + hash;

  for (const ext of readmeExtensions) {
    if (existsSync(absoluteValue + ext)) {
      return absoluteValue + ext + hash;
    }
  }
  return absoluteValue + hash;
}

/**
 * FORKED NOTICE: This function has been modified to support finding files
 * @param {string} value
 * @param {{urlConfig: UrlConfig, path: string, base: string, root: string|undefined, basePath: string|undefined}} config
 * @param {string} type
 */
...
function urlToPath(value, config, type) {
  ...
  if (value.charAt(0) === slash) {
    if (config.basePath) {
      value = findFile(value, config.root, config.basePath);
    } else if (!config.urlConfig.hostname) {
      return
    } else {
      // Create a URL.
